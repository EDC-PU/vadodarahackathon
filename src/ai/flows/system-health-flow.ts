    ...
    async () => {
        ...
        if (serviceAccount.success) {
            try {
                const db = getAdminDb();
                if (!db) {
                    firestore.message = "Firestore admin database is not initialized.";
                    console.error("Firestore admin database is not initialized.");
                } else {
                    const testDocRef = db.collection('healthchecks').doc('test');
                    // Write
        ...
        if (serviceAccount.success) {
            try {
                ...
                if (!authAdmin) {
                    auth.message = "Firebase Auth admin is not initialized.";
        ...
        if (serviceAccount.success) {
            try {
                ...
                if (!storageAdmin) {
                    storage.message = "Firebase Storage admin is not initialized.";
                    console.error("Firebase Storage admin is not initialized.");
                } else {
